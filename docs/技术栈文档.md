...
端到端流程：患者主诉 -> 高风险规则 -> 带版本的检索证据 -> 分诊卡 / 人工选科降级。分诊结果是非诊断结论；命中高风险规则会跳过检索与模型调用。`0.72` 是证据相关性阈值，不代表医疗置信度。MySQL V2-V5 迁移必须完成真实数据库验证，作为发布前置条件。知识重载接口为 ADMIN-only `POST /api/v1/admin/knowledge/reload`，公开 `/api/knowledge/reload` 已下线。
...
已有旧库先备份数据并按顺序执行 `V2__secure_appointments.sql`、`V3__roles_and_doctor_accounts.sql`、`V4__appointment_lifecycle.sql`：V2 补充预约用户/排班归属，V3 增加角色与医生账号映射，V4 增加状态和医生快照并将历史记录标为 `LEGACY`。迁移可重复执行，不创建旧医生的登录账号。真实凭据只通过 `DASH_SCOPE_API_KEY`、`MYSQL_PASSWORD`、JWT secret 等环境变量或被忽略的 `secrets.local.txt` 提供。已有演示数据库如果只有历史日期排班，再使用 `mysql --default-character-set=utf8mb4 -uroot -p guiguxiaozhi -e "source src/main/resources/db/refresh-demo-schedules.sql"` 补充今天起未来 7 天的缺失排班。新数据库仍使用 `src/main/resources/db/init-data.sql`。
