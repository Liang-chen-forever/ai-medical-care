...
本阶段保留 Spring Boot 单体、MyBatis-Plus、MySQL、Redis、MongoDB、LangChain4j、Vue 3 和 uni-app。不引入微服务、消息队列、分布式锁或真实临床数据；这些复杂度不服务于实习项目的可验证目标。

## 2. 核心决策

### 2.1 角色与界面

- `PATIENT`：小程序和 Web 患者端。创建分诊记录、预约、候补、查看就诊摘要和评价。
...
患者端新增“诊前分诊”结果页，展示风险、科室、时效、证据、预约和候补入口。小程序先覆盖患者主路径；Web 患者端保持同一 API。
