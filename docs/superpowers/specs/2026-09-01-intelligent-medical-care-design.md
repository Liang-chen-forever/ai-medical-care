...
面向 Java 后端与全栈实习，将现有项目升级为可稳定演示的“智能导诊与预约挂号平台”。项目继续采用 Spring Boot 单体应用、Vue 3 管理端和 uni-app 微信小程序；重点展示分层设计、鉴权、事务并发控制、缓存一致性、AI 应用安全与部署能力。
...
- API 仅允许受控的跨域来源；微信小程序生产环境配置 HTTPS 合法域名。
...
响应格式固定为 `code`、`message`、`data`；验证失败、未认证、无权限、号源不足和重复预约有稳定错误码。小程序与 Web 共享此契约。

## 微信小程序交付

uni-app 小程序提供登录/注册、科室医生、排班查询、预约、我的预约和 AI 导诊页面。请求层自动携带 token，遇到 401 清空本地登录态并跳转登录页。

小程序的 `config` 集中定义 API 基地址：开发者工具使用电脑局域网 IP，真机/生产使用已配置 HTTPS 的域名。AI 使用普通 `POST` 获取完整响应，Web 端可继续使用 SSE 流式展示。

通过 `npm run dev:mp-weixin` 生成 `dist/dev/mp-weixin`，在微信开发者工具导入该目录进行预览；开发环境需要关闭“不校验合法域名”，生产环境必须配置小程序合法 HTTPS 域名。
...
- README 提供环境准备、密钥配置、数据库初始化、后端/前端/小程序启动和微信开发者工具导入步骤。
