...
The list test now captures the mapper `QueryWrapper`, extracts the parameter key used by the rendered `patient_id = ...` equality predicate, and verifies that exact key resolves to the requested patient value `7L`; it also checks descending `created_at` and `LIMIT 20`. Historical red-state TDD evidence remains unavailable and is intentionally documented as such.

Follow-up fix: preserved original SQL casing while using case-insensitive placeholder matching, avoiding null lookups caused by lowercasing MyBatis-Plus parameter keys. The prior focused run exposed this failure; the corrected test should be rerun in the approved network-enabled Maven environment.
