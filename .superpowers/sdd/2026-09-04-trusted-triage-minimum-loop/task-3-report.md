...
The list test now captures the mapper `QueryWrapper` and verifies the requested patient value `7L` is bound in the equality predicate, plus descending `created_at` and `LIMIT 20` clauses. Historical red-state TDD evidence remains unavailable and is intentionally documented as such.
